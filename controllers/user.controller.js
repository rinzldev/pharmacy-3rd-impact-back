...
async function getUserByID(req, res) {
  try {
    ...
    if (isNaN(uid)) {
      ...
      user = await MUser.findOne({
        where: {
          [Op.or]: [{ UID: uid }, { identification: uid }],
          status: true,
        },
      });
    }
...
async function createUser(req, res) {
  try {
    ...
    const existuser = await MUser.findOne({
      where: {
    ...
    if (existuser) {
      ...
      const FUser = await MUser.findOne({
        where: {
          identification: userData.identification,
          status: false,
        },
      ...
      responses.makeResponsesOk(res, "UCreated");
    }
  } catch (e) {
    responses.makeResponsesException(res, e);
  }
...
async function login(req, res) {
  try {
    ...
    const passwval = await validate.comparePassword(
      req.body.password,
      userval.password
    );
    if (!passwval) {
